...
def Main(animal_folder, family_file, name, death_list = [], has_pet = [], retention = [], age = 0):
    ...
    return has_pet, retention, prop_has_pet, Average_Retention, Average_Compatibility_Score
            ...
            if aging[ii] != 0:
                ...
                for i in range(Num_Trials):
                    ...
                    if(age_aoo_List[i] > 80):
                        print(year_death_list[0][ii][i])
                        year_death_list[0][ii][i] = True
                        print(year_death_list)
                        age_aoo_List[i] = 80
...
def Create_Fam(num_fam, country, animal, aging = [0], year = [0]):
    ...
    for ii in range(len(aging)):
        ...
        for i in range(len(year)):
            ...
            df.iloc[7] = pk[ii][i]
            df.iloc[8] = aoo[ii][i]
            df.insert(0, "Death", [0]*9)
            df.to_csv(country + "/Simulation Family " + str(aging[ii]) + " Years Later For " + str(year[i]) + " Years Afterwards.csv", index=False)    
            list1, list2, propretention, avgretention, averagescore = Main(animal, country + "/Simulation Family " + str(aging[ii]) + " Years Later For " + 
